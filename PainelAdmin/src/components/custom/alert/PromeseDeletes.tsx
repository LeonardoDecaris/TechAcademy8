...
function successMsg(data: any) {
  return (
    ...
    data?.mensagem ||
    data?.data?.message ||
    "Delete realizado com sucesso, Aguarde alguns instantes"
  );
}
...
export function deleteToast<T>(fn: () => Promise<T>, opts?: ToastPromiseOptions) {
  return toast.promise(fn(), {
    loading: opts?.loading ?? "Deletando...",
    success:
